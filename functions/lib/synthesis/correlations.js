...
exports.getCorrelationStrength = getCorrelationStrength;
 ...
 */
function interpretCorrelation(r, outcome) {
    const absR = Math.abs(r);
    const expectedDirection = types_1.OUTCOME_EXPECTED_DIRECTION[outcome];
    const metricName = types_1.OUTCOME_METRIC_NAMES[outcome];
    // Determine direction
    let direction;
    if (absR < 0.1) {
        direction = 'neutral';
    ...
    if (direction === 'neutral') {
        ...
        const isBeneficial = (direction === 'positive' && expectedDirection === 'positive') ||
