 ...
 */
function isMorningAnchorProtocol(protocolId) {
    return MORNING_ANCHOR_PROTOCOL_IDS.some((id) => protocolId.toLowerCase().includes(id.toLowerCase()));
}
 ...
 */
async function getTodayNudgeStats(firestore, userId) {
    const now = new Date();
...
const generateAdaptiveNudges = async (_event, _context) => {
    ...
    `;
        ...
        const memories = await (0, memory_1.getRelevantMemories)(userId, {
        ...
        const scoredProtocols = ragResults.map((protocol) => {
            ...
            return { protocol, confidence: (0, reasoning_1.calculateConfidence)(nudgeContext) };
        });
        ...
        let mvdState = await (0, mvd_1.getMVDState)(userId);
        // Check exit condition first if MVD is active
        if (mvdState?.mvd_active) {
            const exited = await (0, mvd_1.checkAndMaybeExitMVD)(userId, recoveryScore);
            if (exited) {
        ...
        if (!mvdState?.mvd_active) {
            const mvdContext = await (0, mvd_1.buildMVDDetectionContext)(userId, userTimezone, // Use as device timezone (best available)
            false // Not manual activation
            );
            const detection = await (0, mvd_1.detectAndMaybeActivateMVD)(mvdContext);
            if (detection.wasActivated) {
                console.log(`[NudgeEngine] MVD activated for user ${userId}: ` +
                    `trigger=${detection.trigger}, type=${detection.mvdType}`);
                mvdState = await (0, mvd_1.getMVDState)(userId);
            }
        }
        ...
        const suppressionContext = (0, suppression_1.buildSuppressionContext)({
            ...
            userLocalHour: (0, suppression_1.getUserLocalHour)(new Date(), userTimezone),
            userPreferences: {
        ...
        if (!suppressionResult.shouldDeliver) {
            ...
            console.log(`[NudgeEngine] Nudge suppressed for user ${userId}: ` +
    ...
    `;
        // Generate Nudge
        let nudgeText = await (0, vertexAI_1.generateCompletion)(SYSTEM_PROMPT, userPrompt);
        ...
        const nudgeScan = (0, safety_1.scanAIOutput)(nudgeText, 'nudge');
        if (!nudgeScan.safe) {
