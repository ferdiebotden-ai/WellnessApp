...
exports.getLatestRecoveryScore = getLatestRecoveryScore;
...
exports.getMeetingHoursToday = getMeetingHoursToday;
exports.buildMVDDetectionContext = buildMVDDetectionContext;
exports.hasSufficientDataForMVD = hasSufficientDataForMVD;
 ...
 */
async function getLatestRecoveryScore(userId) {
 ...
 */
async function getCompletionHistory(userId, days = types_1.MVD_CONFIG.CONSISTENCY_DAYS) {
    const supabase = (0, supabaseClient_1.getServiceClient)();
 ...
 */
async function getMeetingHoursToday(userId) {
    try {
 ...
 */
async function buildMVDDetectionContext(userId, deviceTimezone = null, isManualActivation = false) {
    // Fetch all data in parallel (including calendar data)
    const [recoveryScore, completionHistory, userTimezone, meetingHoursToday] = await Promise.all([
        getLatestRecoveryScore(userId),
        getCompletionHistory(userId, types_1.MVD_CONFIG.CONSISTENCY_DAYS),
 ...
 */
function hasSufficientDataForMVD(context) {
