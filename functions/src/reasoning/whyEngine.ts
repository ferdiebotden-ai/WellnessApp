...
import { ConfidenceScore, EvidenceLevel, EVIDENCE_SCORES } from './types';
 ...
 */
export function extractMechanism(description: string | null): string {
  if (!description) {
 ...
 */
export function parseDOI(citation: string): string | undefined {
 ...
 */
export function mapConfidenceLevel(score: number): ConfidenceLevel {
 ...
 */
export async function generateYourData(
  protocol: ProtocolSearchResult,
  memories: ScoredMemory[],
  confidence: ConfidenceScore
): Promise<string> {
...
`;
  ...
  const systemPrompt =
    'You are a wellness data analyst. Generate brief, specific, personalized insights. Never exceed 140 characters.';

  try {
    const result = await generateCompletion(systemPrompt, prompt, 0.5);
    // Clean and validate
...
function getEvidenceLevel(
  ...
  const protocolWithEvidence = protocol as ProtocolSearchResult & {
  ...
  if (level && EVIDENCE_SCORES[level] !== undefined) {
    return level;
  }
 ...
 */
export async function generateWhyExpansion(
  params: WhyExpansionParams
): Promise<WhyExpansion> {
