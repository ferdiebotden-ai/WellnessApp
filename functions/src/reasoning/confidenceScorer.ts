 ...
 * - No match: 0.3
 */
function calculateProtocolFit(context: NudgeContext): number {
  const { primary_goal, module_id, protocol } = context;

  const optimalModules = GOAL_MODULE_MAPPING[primary_goal] || [];
  ...
  const keywords = GOAL_KEYWORDS[primary_goal] || [];
 ...
 */
function calculateMemorySupport(context: NudgeContext): number {
  ...
  for (const memory of memories) {
    ...
    const isProtocolRelated =
      memory.source_protocol_id === protocol.id ||
 ...
 */
function calculateTimingFit(context: NudgeContext): number {
  ...
  const optimalTimes = CATEGORY_TIME_MAPPING[category] || ['morning', 'afternoon', 'evening'];
 ...
 */
function calculateConflictRisk(context: NudgeContext): number {
  ...
  for (const other of other_protocols) {
    if (other.id === protocol.id) continue;
    ...
    if (
      (thisName.includes('caffeine') && otherName.includes('sleep')) ||
    ...
    if (
      (thisName.includes('cold') && otherName.includes('fitness')) ||
 ...
 */
function calculateEvidenceStrength(context: NudgeContext): number {
  const { protocol } = context;

  // Get evidence level from protocol metadata
  const evidenceLevel = (protocol as ProtocolSearchResult & { evidence_level?: EvidenceLevel })
    .evidence_level;

  if (evidenceLevel && EVIDENCE_SCORES[evidenceLevel] !== undefined) {
    return EVIDENCE_SCORES[evidenceLevel];
  }

  // Default to 'High' if not specified (most protocols in the library are High)
  return EVIDENCE_SCORES['High'];
}
...
function generateReasoning(
  ...
  } else if (overall >= CONFIDENCE_SUPPRESSION_THRESHOLD) {
 ...
 */
export function calculateConfidence(context: NudgeContext): ConfidenceScore {
  ...
  for (const [factor, weight] of Object.entries(CONFIDENCE_WEIGHTS)) {
    overall += factors[factor as keyof ConfidenceFactors] * weight;
  }
  ...
  const should_suppress = overall < CONFIDENCE_SUPPRESSION_THRESHOLD;
