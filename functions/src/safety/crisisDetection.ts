...
import { SAFETY_CONFIG, SEVERITY_PRIORITY } from './types';
import {
  CRISIS_KEYWORDS,
 ...
 */
export function hasExclusionMatch(text: string, exclusions: string[]): boolean {
 ...
 */
export function findMatchingKeywords(text: string): CrisisKeyword[] {
...
export function detectCrisis(text: string): CrisisDetectionResult {
  ...
  if (matchedKeywordObjects.length === 0) {
    return {
      ...
      matchedKeywords: [],
      resources: [],
      shouldLog: SAFETY_CONFIG.LOG_ALL_SCANS,
    };
  }
  ...
  let resourceLimit: number;
  switch (highestSeverity) {
    case 'high':
      resourceLimit = SAFETY_CONFIG.HIGH_SEVERITY_RESOURCE_COUNT;
      break;
    case 'medium':
      resourceLimit = SAFETY_CONFIG.MEDIUM_SEVERITY_RESOURCE_COUNT;
      break;
    default:
      resourceLimit = SAFETY_CONFIG.LOW_SEVERITY_RESOURCE_COUNT;
  }

  const resources = highestSeverity
    ? getResourcesForSeverity(highestSeverity, resourceLimit)
    : [];
