 ...
 */
export function getResourcesForSeverity(
  severity: CrisisSeverity,
  limit?: number
): CrisisResource[] {
  const filtered = CRISIS_RESOURCES.filter((r) =>
    r.forSeverities.includes(severity)
...
export function getHighestSeverity(
  ...
  return severities.reduce((highest, current) =>
    SEVERITY_PRIORITY[current] > SEVERITY_PRIORITY[highest] ? current : highest
  );
}
 ...
 */
export function getUniqueSeverities(keywords: CrisisKeyword[]): CrisisSeverity[] {
