...
import { pearsonCorrelation, interpretCorrelation } from './correlations';
import {
  WeeklyMetrics,
  ProtocolBreakdown,
  ProtocolCorrelation,
...
export async function aggregateWeeklyMetrics(
  ...
  correlationLookbackStart.setDate(
    correlationLookbackStart.getDate() - SYNTHESIS_CONFIG.CORRELATION_LOOKBACK_DAYS
  );
  ...
  const [
    currentWeekLogsResult,
    priorWeekLogsResult,
    protocolNamesResult,
    wearableDataResult,
    enrolledProtocolsResult,
    correlationLogsResult,
    correlationWearableResult,
  ] = await Promise.all([
  ...
  const currentWeekLogs = (currentWeekLogsResult.data || []) as ProtocolLogRow[];
  const priorWeekLogs = (priorWeekLogsResult.data || []) as ProtocolLogRow[];
  const protocolNames = (protocolNamesResult.data || []) as ProtocolNameRow[];
  const wearableData = wearableDataResult as WearableDataRow[];
  const enrolledProtocols = (enrolledProtocolsResult.data || []) as EnrolledProtocolRow[];
  const correlationLogs = (correlationLogsResult.data || []) as ProtocolLogRow[];
  const correlationWearable = correlationWearableResult as WearableDataRow[];

  // Build protocol name lookup map
  const protocolNameMap = new Map(protocolNames.map((p) => [p.id, p.name]));

  // Calculate all metrics
  const protocolBreakdown = calculateProtocolBreakdown(
    currentWeekLogs,
    protocolNameMap
  );
  ...
  const weekOverWeek = calculateWeekOverWeek(
    ...
    protocolNameMap,
    wearableData,
    await fetchWearableData(supabase, userId, priorStartStr, priorEndStr)
  );
...
async function fetchWearableData(
  ...
  try {
    ...
    return (data || []) as WearableDataRow[];
  } catch (err) {
 ...
 */
function calculateProtocolBreakdown(
  logs: ProtocolLogRow[],
  protocolNameMap: Map<string, string>
): ProtocolBreakdown[] {
  const byProtocol = new Map<
    string,
    { days: Set<string>; totalCompletions: number; durations: number[] }
  >();
 ...
 */
function calculateDaysWithCompletion(logs: ProtocolLogRow[]): number {
...
function calculateAdherence(
  logs: ProtocolLogRow[],
  enrolledProtocols: EnrolledProtocolRow[],
  protocolNameMap: Map<string, string>
): number {
  ...
  const protocolDays = new Set<string>();
  for (const log of logs) {
 ...
 */
function calculateWearableMetrics(wearableData: WearableDataRow[]): {
  ...
  const hrvTrend = calculateTrendPercent(
    wearableData.map((d) => d.hrv_score)
  );
  const sleepTrend = calculateTrendPercent(
    wearableData.map((d) => d.sleep_hours)
  );

  // Count unique days with data
  const uniqueDays = new Set(
    wearableData.map((d) => d.recorded_at.split('T')[0])
  );
 ...
 */
function calculateTrendPercent(values: (number | null)[]): number | null {
...
function calculateWeekOverWeek(
  currentLogs: ProtocolLogRow[],
  priorLogs: ProtocolLogRow[],
  enrolledProtocols: EnrolledProtocolRow[],
  protocolNameMap: Map<string, string>,
  currentWearable: WearableDataRow[],
  priorWearable: WearableDataRow[]
): WeekOverWeekComparison {
  // Calculate adherence for both weeks
  const currentAdherence = calculateAdherence(
    currentLogs,
    enrolledProtocols,
    protocolNameMap
  );
  const priorAdherence = calculateAdherence(
    priorLogs,
    enrolledProtocols,
    protocolNameMap
  );
  ...
  const recoveryChange =
    currentRecovery !== null && priorRecovery !== null
...
function calculateCorrelations(
  logs: ProtocolLogRow[],
  wearableData: WearableDataRow[],
  protocolNameMap: Map<string, string>
): ProtocolCorrelation[] {
  if (logs.length < SYNTHESIS_CONFIG.MIN_CORRELATION_DAYS || wearableData.length < SYNTHESIS_CONFIG.MIN_CORRELATION_DAYS) {
    return [];
  }
  ...
  const dailyMetrics = new Map<
    string,
    { hrv_score: number | null; sleep_hours: number | null; recovery_score: number | null; resting_hr: number | null }
  >();
  ...
  const protocolDays = new Map<string, Set<string>>();
  for (const log of logs) {
  ...
  const allDays = Array.from(
    new Set([...dailyMetrics.keys(), ...Array.from(protocolDays.values()).flatMap((s) => Array.from(s))])
  ).sort();
  ...
  for (const [protocolId, completedDays] of protocolDays) {
    ...
    for (const outcome of outcomes) {
      ...
      if (protocolArray.length < SYNTHESIS_CONFIG.MIN_CORRELATION_DAYS) continue;

      try {
        const result = pearsonCorrelation(protocolArray, outcomeArray);
        const { direction, interpretation } = interpretCorrelation(result.r, outcome);
        ...
        correlations.push({
          ...
          is_significant: result.p_value < SYNTHESIS_CONFIG.CORRELATION_P_THRESHOLD,
