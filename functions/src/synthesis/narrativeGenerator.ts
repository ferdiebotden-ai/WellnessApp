...
import { generateCompletion } from '../vertexAI';
import { scanAIOutput, getSafeFallbackResponse } from '../safety';
import { getRelevantMemories, type ScoredMemory } from '../memory';
import {
  WeeklyMetrics,
 ...
 */
function detectSections(narrative: string): NarrativeSection[] {
  const lowerNarrative = narrative.toLowerCase();
  const detected: NarrativeSection[] = [];

  // Keywords that indicate each section
  const sectionIndicators: Record<NarrativeSection, string[]> = {
...
function generateFallbackNarrative(
  ...
  if (metrics.data_days_available < SYNTHESIS_CONFIG.MIN_DATA_DAYS) {
    return `${user.display_name}, your Weekly Brief needs a bit more data to generate meaningful insights. ` +
      `You've logged ${metrics.data_days_available} days this week — just ${SYNTHESIS_CONFIG.MIN_DATA_DAYS - metrics.data_days_available} more days of tracking ` +
...
export async function generateWeeklyNarrative(
  ...
  const userMemories = memories ?? await getRelevantMemories(
    metrics.user_id,
    { memory_types: ['pattern_detected', 'protocol_effectiveness'], min_confidence: 0.2 },
    5
  );
  ...
  try {
    narrative = await generateCompletion(SYSTEM_PROMPT, userPrompt, 0.7);
  } catch (err) {
  ...
  const safetyScan = scanAIOutput(narrative, 'ai_response');
  if (!safetyScan.safe) {
    console.warn('[NarrativeGenerator] Content flagged:', safetyScan.reason);
    narrative = getSafeFallbackResponse('ai_response');
  }
  ...
  // Log quality metrics
  console.log(
    `[NarrativeGenerator] Generated for ${metrics.user_id}: ` +
 ...
 */
export function validateNarrative(result: WeeklySynthesisResult): {
  valid: boolean;
  issues: string[];
} {
  const issues: string[] = [];
  ...
  // Check word count
  if (result.word_count < SYNTHESIS_CONFIG.MIN_WORD_COUNT) {
    issues.push(`Word count too low: ${result.word_count} < ${SYNTHESIS_CONFIG.MIN_WORD_COUNT}`);
  }
  if (result.word_count > SYNTHESIS_CONFIG.MAX_WORD_COUNT) {
    issues.push(`Word count too high: ${result.word_count} > ${SYNTHESIS_CONFIG.MAX_WORD_COUNT}`);
  }
