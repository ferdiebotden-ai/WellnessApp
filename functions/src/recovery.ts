...
import { DailyMetricsRow } from './types/wearable.types';
import { CheckInResult, CheckInPayload } from './types/checkIn.types';
 ...
 */
function getBaselineStatus(baseline: UserBaseline | null): BaselineStatusResponse {
  if (!baseline) {
    return {
      ready: false,
      daysCollected: 0,
  ...
  const confidenceLevel = determineBaselineConfidence(sampleCount);
 ...
 */
function getLiteModeBaselineStatus(): BaselineStatusResponse {
  return {
    ready: true,
    daysCollected: 0,
    daysRequired: 0,
 ...
 */
function transformRecoveryResult(result: RecoveryResult): RecoveryScoreResponse {
  return {
    score: result.score,
 ...
 */
function transformCheckInResult(result: CheckInResult): CheckInApiResponse {
  return {
    score: result.score,
    confidence: result.confidence,
    zone: result.zone,
 ...
 */
export async function getRecoveryScore(req: Request, res: Response): Promise<void> {
  if (req.method !== 'GET') {
  ...
  try {
    ...
    const wearableSource = user.wearable_source as string | null;
    ...
    if (isLiteMode) {
      ...
      if (metrics && metrics.wearable_source === 'manual' && metrics.raw_payload) {
        ...
        if (payload.type === 'manual_check_in' && payload.result) {
          const response: RecoveryApiResponse = {
    ...
    const baseline = baselineRow ? fromUserBaselineRow(baselineRow as UserBaselineRow) : null;
    ...
    const recoveryResult = calculateRecoveryScore({
      dailyMetrics: metrics as DailyMetricsRow,
